...
class Rate_Validator
	...
	def initialize()
		...
		File.delete("#{$rate_validation_path}/#{@efn}_validationResults.csv") if File.exists?("#{$rate_validation_path}/#{@efn}_validationResults.csv")
		...
		if efns != nil
			efns.each do |eachEFN| #Set EFN and compare for each EFN/rateCheck file in /ref/
				@efn = eachEFN
				compare()
				puts "Validation results generated and stored in #{$rate_validation_path}/#{@efn}_validationResults.csv!" if File.exists?("#{$rate_validation_path}/#{@efn}_validationResults.csv")
				puts "Generation of #{$rate_validation_path}/#{@efn}_validationResults.csv failed.." if not File.exists?("#{$rate_validation_path}/#{@efn}_validationResults.csv")
			end
		end
	...
	def getEFN()
		...
		puts "Type 'auto' to automatically search #{$rate_validation_path}/ for Rate Check files ([efn]_rateCheckXX.csv).."
		...
		if efn.downcase == 'auto'
				...
				varianceFiles = Dir.glob("#{$rate_validation_path}/*_variance*")
				...
				if bothFilesExist
					...
					puts "1) Both the Rate Check and Variance files are in the correct location (#{$rate_validation_path}/)."
			...
			bench.report do
				variance.each do |varRecord|
					...
					if checkRecord
						...
						checkRecord['RI'] != varRecord['Rate'] ? rate_indicator = "#{varRecord['Rate']}[#{checkRecord['RI']}]" : rate_indicator = varRecord['Rate']
						if checkRecord['Mail Class'].domestic?
							comparedRecords << ["'#{varRecord['PIC']}'", varRecord['Mail Class'], checkRecord['PC'], rate_indicator, varRecord['Dest Rate'], checkRecord["Discount/Surcharge"], checkRecord['Barcode'], varRecord['Weight'], checkRecord['Zone'], varRecord['eVS Zone'], checkRecord['Base Rate'], checkRecord['Plus Rate'], varRecord['eVS Postage + Surch ($)'], "Validated at #{matchedTier}"] if matched
							comparedRecords << ["'#{varRecord['PIC']}'", varRecord['Mail Class'], checkRecord['PC'], rate_indicator, varRecord['Dest Rate'], checkRecord["Discount/Surcharge"], checkRecord['Barcode'], varRecord['Weight'], checkRecord['Zone'], varRecord['eVS Zone'], checkRecord['Base Rate'], checkRecord['Plus Rate'], varRecord['eVS Postage + Surch ($)'], "Matching rate not found."] unless matched
						else
							comparedRecords << ["'#{varRecord['PIC']}'", varRecord['Mail Class'], checkRecord['PC'], rate_indicator, varRecord['Dest Rate'], checkRecord["Discount/Surcharge"], checkRecord['Barcode'], varRecord['Weight'], checkRecord['Price Group'], varRecord['eVS Zone'], checkRecord['Base Rate'], checkRecord['Plus Rate'], varRecord['eVS Postage + Surch ($)'], "Validated at #{matchedTier}"] if matched
	...
	def resultsAlreadyExist?(efn)
		...
		existingValidationResults = Dir.glob("#{$rate_validation_path}/#{efn}_validationResults.csv")
	...
	def loadFile(*param) #param[0] is type with expected values 'Rate Check', 'Variance', and 'Sample'. param[1] is a file name (used to pass in sampling report for current month)
		...
		fileNames = Dir.glob("#{$rate_validation_path}/#{@efn}_rateCheck??.csv") if param[0] == 'Rate Check' #Should only add 1 file name to the array fileName.
		fileNames = Dir.glob("#{$rate_validation_path}/#{@efn}_variance*.csv") if param[0] == 'Variance' #Dir.glob returns an array of all matching files..can be more than 1.
